...
def image_dict_from_any(image) -> Optional[Dict[str, np.ndarray]]:
    ...
    if "mask" not in image or image["mask"] is None:
        ...
        if os.path.exists(image["mask"]):
            image["mask"] = np.array(Image.open(image["mask"]).convert("RGB")).astype(
                "uint8"
            )
        elif image["mask"]:
...
def get_control(
    ...
    if isinstance(input_image, list):
        ...
        input_image = Script.try_crop_image_with_a1111_mask(
    ...
    def preprocess_input_image(input_image: np.ndarray):
        ...
        result = preprocessor.cached_call(
            ...
            low_vram=(
                ("clip" in unit.module or unit.module == "ip-adapter_face_id_plus")
    ...
    controls, hr_controls = list(
        zip(*[preprocess_input_image(img) for img in optional_tqdm(input_images)])
    )
...
class Script(
    ...
    ) -> Tuple[ControlNetUiGroup, gr.State]:
    ...
    def ui_batch_options(self, is_img2img: bool, elem_id_tabname: str):
        ...
        for comp in unit_args:
            ...
            for event_subscriber in event_subscribers:
                event_subscriber(fn=update_ui_batch_options, inputs=unit_args)
    ...
    def ui(self, is_img2img):
        ...
        with gr.Group(elem_id=elem_id_tabname):
            with gr.Accordion(
                f"ControlNet {controlnet_version.version_flag}",
                open=False,
                elem_id="controlnet",
            ):
                photopea = (
                ...
                if max_models > 1:
                    with gr.Tabs(elem_id=f"{elem_id_tabname}_tabs"):
                        for i in range(max_models):
                            with gr.Tab(
                                f"ControlNet Unit {i}", elem_classes=["cnet-unit-tab"]
                            ):
                                group, state = self.uigroup(
                                    f"ControlNet-{i}",
                                    is_img2img,
                                    elem_id_tabname,
                                    photopea,
                    ...
                    with gr.Column():
                        group, state = self.uigroup(
                            "ControlNet", is_img2img, elem_id_tabname, photopea
                        )
                        ui_groups.append(group)
                        controls.append(state)
                with gr.Accordion(
                    "Batch Options", open=False, elem_id="controlnet_batch_options"
                ):
    ...
    def try_crop_image_with_a1111_mask(
        ...
        is_upscale_script = any(
            "upscale" in k.lower()
            for k in getattr(p, "extra_generation_params", {}).keys()
        )
        ...
        if (
            ...
            input_image = [
                images.resize_image(
                    ResizeMode.OUTER_FIT.int_value(), x, p.width, p.height
                )
    ...
    def check_sd_version_compatible(unit: external_code.ControlNetUnit) -> None:
        ...
        if (
            "revision" in unit.module.lower()
            and sd_version != StableDiffusionVersion.SDXL
        ):
            raise Exception(
        ...
        if cnet_sd_version == StableDiffusionVersion.UNKNOWN:
            logger.warn(
                f"Unable to determine version for ControlNet model '{unit.model}'."
            )
            return
    ...
    def controlnet_main_entry(self, p):
        ...
        logger.info(
            f"unit_separate = {batch_option_uint_separate}, style_align = {batch_option_style_align}"
        )
        ...
        Preprocessor.unload_unused(
            active_processors={
                unit.get_actual_preprocessor() for unit in self.enabled_units
            }
        )
        high_res_fix = isinstance(p, StableDiffusionProcessingTxt2Img) and getattr(
            p, "enable_hr", False
        ...
        for idx, unit in enumerate(self.enabled_units):
            ...
            if (
                ...
                logger.warning(
                    "A1111 inpaint and ControlNet inpaint duplicated. Falls back to inpaint_global_harmonious."
                )
                unit.module = "inpaint"
            ...
            if preprocessor.do_not_need_model:
                ...
                model_net, control_model_type = Script.load_control_model(
                    p, unet, unit.model
                )
                model_net.reset()

                if (
                    model_net is not None
                    and getattr(devices, "fp8", False)
                    and control_model_type == ControlModelType.ControlNet
                ):
                    for (
                        _module
                    ) in (
                        model_net.modules()
                    ):  # FIXME: let's only apply fp8 to ControlNet for now
            ...
            if unit.effective_region_mask is not None:
                assert (
                    control_model_type.supports_effective_region_mask
                ), f"`effective_region_mask` not supported for {control_model_type}"
            ...
            if len(controls) == len(hr_controls) == 1 and control_model_type not in [
                ControlModelType.SparseCtrl
            ]:
                control = controls[0]
                hr_control = hr_controls[0]
            elif unit.is_animate_diff_batch or control_model_type in [
                ControlModelType.SparseCtrl
            ]:
                cn_ad_keyframe_idx = getattr(unit, "batch_keyframe_idx", None)

                def ad_process_control(
                    cc: List[torch.Tensor], cn_ad_keyframe_idx=cn_ad_keyframe_idx
                ):
                    ...
                    if (
                        ...
                        and model_net.control_model.use_simplified_condition_embedding
                    ):  # noqa
                        c = UnetHook.call_vae_using_process(p, c)
                    # handle key frame control for different control methods
                    if cn_ad_keyframe_idx is not None or control_model_type in [
                        ControlModelType.SparseCtrl
                    ]:
                        if control_model_type == ControlModelType.SparseCtrl:
                            ...
                            if cn_ad_keyframe_idx is None:
                                ...
                                logger.info(
                                    f"SparseCtrl: control images will be applied to frames: {cn_ad_keyframe_idx}"
                                )
                            else:
                                logger.info(
                                    f"SparseCtrl: control images will be applied to frames: {cn_ad_keyframe_idx}"
                                ...
                                for frame_idx, frame_path in zip(
                                    unit.batch_keyframe_idx, unit.batch_image_files
                                ):
                                    logger.info(f"\t{frame_idx}: {frame_path}")
                            c = SparseCtrl.create_cond_mask(
                                cn_ad_keyframe_idx, c, p.batch_size
                            ).cpu()
                        elif unit.accepts_multiple_inputs:
                            # ip-adapter should do prompt travel
                            logger.info(
                                "IP-Adapter: control prompts will be traveled in the following way:"
                            )
                            for frame_idx, frame_path in zip(
                                unit.batch_keyframe_idx, unit.batch_image_files
                            ):
                            ...
                            prompt_scheduler.prompt_map = {
                                i: "" for i in cn_ad_keyframe_idx
                            }
                            prompt_closed_loop = (
                                ad_params.video_length > ad_params.batch_size
                            ) and (ad_params.closed_loop in ["R+P", "A"])
                            c_full = prompt_scheduler.multi_cond(
                                c_full, prompt_closed_loop
                            )
                            ...
                            logger.info(
                                f"ControlNet: control images will be applied to frames: {cn_ad_keyframe_idx} where"
                            )
                            for frame_idx, frame_path in zip(
                                unit.batch_keyframe_idx, unit.batch_image_files
                            ):
                    ...
                    if (
                        shared.opts.batch_cond_uncond
                        and not unit.accepts_multiple_inputs
                    ):
                ...
                hr_control = (
                    ad_process_control(hr_controls)
            ...
            global_average_pooling = (
                control_model_type.is_controlnet
                and model_net.control_model.global_average_pooling
            )
        ...
        for i, (param, unit) in enumerate(zip(forward_params, self.enabled_units)):
            if param.control_model_type == ControlModelType.IPAdapter:
                if param.advanced_weighting is not None:
                    logger.info(
                        f"IP-Adapter using advanced weighting {param.advanced_weighting}"
                    )
                    assert (
                        len(param.advanced_weighting)
                        == global_state.get_sd_version().transformer_block_num
                    )
                    # Convert advanced weighting list to dict
                    weight = {
                        i: w for i, w in enumerate(param.advanced_weighting) if w > 0
                    }
                else:
                    weight = param.weight
            ...
            if param.control_model_type == ControlModelType.InstantID:
                ...
                assert (
                    ip_adapter_param.control_model_type == ControlModelType.IPAdapter
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            "",
            "Path to directory containing annotator model directories (overrides corresponding command line flag)",
            section=section,
        ).needs_reload_ui(),
    )
    shared.opts.add_option(
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            7,
            "ControlNet inpainting Gaussian blur sigma",
            gr.Slider,
    ...
    shared.opts.add_option(
        "controlnet_clip_detector_on_cpu",
        shared.OptionInfo(
            False,
            "Load CLIP preprocessor model on CPU",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
    ...
    shared.opts.add_option(
        "controlnet_control_type_dropdown",
        shared.OptionInfo(
            False,
            "Display control type as dropdown",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ).needs_reload_ui(),
